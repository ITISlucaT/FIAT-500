...
model_lat_lon = joblib.load("Linear_regressor_FIAT500_lat-lon_included.pkl")
encoders = joblib.load("label_encoders.pkl")
StandardScaler_X  = joblib.load("standard_scaler.pkl")
StandardScaler_X_lat_lon  = joblib.load("standard_scaler_lat_lon.pkl")
...
@app.route('/predict_lat_long', methods=['POST'])
def predict_lat_long():
    ...
    try:
        ...
        user_input = np.array([model_type, engine_power, transmission, age_in_days, km, previous_owners, lat, lon])
        user_input_model_encoded = LabelEncoder_model.transform(user_input[0].reshape(-1))
        user_input_transmission_encoded = LabelEncoder_transmission.transform(user_input[2].reshape(-1))
        df = pd.DataFrame([user_input], columns=["model", 	"engine_power" ,	"transmission" ,	"age_in_days" 	,"km" 	,"previous_owners", "lat", "lon"])

        user_input_norm = StandardScaler_X_lat_lon.transform(df[["engine_power","age_in_days", "km", "previous_owners", "lat", "lon"]])
        user_input_to_predict = np.insert(user_input_norm, 0, user_input_model_encoded, axis=1)

        user_input_to_predict = np.insert(user_input_to_predict, 2, user_input_transmission_encoded, axis=1)
        prediction = model_lat_lon.predict(user_input_to_predict)
        return jsonify({
            'price': float(prediction[0])
        })
